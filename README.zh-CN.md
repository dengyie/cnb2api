...
## 资源经济学:你到底能免费用多少?(实测硬核账本)

很多开发者平台的所谓「免费额度」跑两天测试就见底。基于我们真实线上长期运行的实测数据,这是 CNB 提供的完整资源明细与精算账本:

### 月度额度明细

| 资源项 | 月度免费配额 | 对应实际价值 | 真实可用时长与吞吐 |
|---|---|---|---|
| **AI 额度 (Credits)** | **500 ~ 1,166 credits** | 价值 ¥500 ~ ¥1,166 的大模型 API 调用额度 | **3 亿 ~ 10 亿+ Tokens** (基于 flash 模型实测单价) |
| **开发核时 (Dev)** | **1,600 核时** (5,760,000 秒) | 等效一台 2 核独立云容器 7×24 不间断运行 | **33.3 天** (完全覆盖单月 30 天 720 小时) |
| **CI 核时 (CI)** | **160 ~ 1,600 核时** | 自动化测试与定时保活流水线 | 每 5 分钟 cron 保活单月仅耗 **~18–20 核时** |

> **如何拿到 1,166 额度**:每个认证组织注册即送 500 额度/月;完成官方「hello-cnb」闯关任务(解锁「天才程序员」身份认证)再加赠 666 额度/月。每月自动重置刷新。

### 吞吐量与 Token 算力精算

- **2 核工作区 33.3 天不间断在线**:默认 `runner.cpus: 2` 的工作区每天消耗 `2 × 24 = 48` 核时。连续跑满 30 天整月只需 `1,440` 核时,还留有 **160 核时的安全余量**。你完全不需要为了省核时而关停反代。
- **单月 3 亿 ~ 10 亿 Tokens 吞吐**:上游旗舰模型 `deepseek-v4-flash` 具备极高的性价比(约 1 credit 可兑换 50 万~100 万 tokens)。1,166 credits 意味着你每天拥有 **1000 万 ~ 3000 万 tokens** 的可用预算——轻松支撑 Cursor/Copilot 重度编程、自主 Agent 复杂任务循环以及全天候文档翻译。
- **定时保活开销近乎为零**:每 5 分钟触发一次的自愈流水线使用轻量容器执行,单次耗时仅 6~8 秒(~0.002 核时),整月保活开销累计不到 20 核时。

### 工作区生命周期与可用性 (99.85% SLA)

- **单次最大运行时间**:最长可达 18 小时 (`options.keepAliveTimeout: 64800000`)。
- **官方回收窗口**:运行超 8 小时的工作区在凌晨 04:00–06:00 (UTC+8) 会被平台统一回收重建。
- **实测自愈速度**:工作区被回收后,5 分钟保活 cron 自动检出并拉起新容器(~35s),工作区开机秒级自注册新子域名,全链路切换耗时仅 **~2 分 13 秒**。
- **真实可用性**:一个月 30 天的累计重启中断窗口约 66 分钟,在完全易逝的免费算力上做出了高达 **99.85% 的可用性 SLA**。

## 支持的模型矩阵

CNB AI 网关直接提供极速推理。默认情况下,cnb2api 已内置常用模型配置:

| 模型标识 (Model ID) | 架构与能力 | 上下文窗口 | 原生工具调用 (`tools`) | 思考过程流 (`reasoning_content`) | 推荐使用场景 |
|---|---|---|:---:|:---:|---|
| `deepseek-v4-flash` *(默认推荐)* | 顶级多模态与高吞吐架构 | 64k ~ 128k | ✅ 支持 | ✅ 完整聚合与透传 | 日常代码补全、自主 Agent 运行、高并发问答 |
| `glm-5.3-flash` *(智能别名)* | 通用高智力路由别名 | 64k ~ 128k | ✅ 支持 | ✅ 完整聚合与透传 | 习惯使用智谱 GLM 命名规范的客户端 |
| `kimi-k3` *(智能别名)* | 超长文本理解智能别名 | 64k ~ 128k | ✅ 支持 | ✅ 完整聚合与透传 | 超长文档阅读、大文件代码分析与论文精读 |
| `deepseek-chat` *(智能别名)* | OpenAI 标准通用别名 | 64k ~ 128k | ✅ 支持 | ✅ 完整聚合与透传 | 开箱即用无缝接入各主流开源客户端 |

所有模型均同时支持 **SSE 流式传输** 与 **非流式响应**,且响应中完整携带 Token 用量统计 (`prompt_tokens`, `completion_tokens`, `total_tokens`)。
...
| `PROXY_MODELS` | `deepseek-v4-flash,glm-5.3-flash,kimi-k3,deepseek-chat` | `/v1/models` 展示的模型 id 列表。 |
