...
这套方案背后有两份相互独立的免费额度:**AI credits** 付推理费,**核时**
付反代算力费。以下数字全部来自我们自己长期在线的部署——是实测,不是营销。

### 两份额度

| 资源 | 每月免费配额 | 用来付什么 |
|---|---|---|
| **AI credits** | 基础 **500**,完成闯关后 **1,166** | 每一次 AI 请求;每个响应的 `usage` 都会上报本次消耗的 `credit` |
| **算力核时** | **1,600 核时**(dev + CI 共享池) | 跑反代的工作区,外加 5 分钟一次的保活流水线 |
...
上游在每次响应的 `usage` 里直接返回 `credit` 字段,消耗是精确值而非估算。
对 `deepseek-v4-flash` 的实测:

- 全新(未缓存)请求约 9,000 tokens 花费 **0.39 credit**——约
  **~23,000 tokens / credit**,多次重复测量一致。
...
- `runner.cpus: 2` 的工作区每天烧 **48 核时**;整月 30 天不间断 = **1,440
  核时**,在 **1,600 核时** 池子之内。不需要为省核时而关停反代。
- 保活流水线每 5 分钟跑一次,单次仅 6~8 秒——对月度池子是零头的零头。

### 工作区生命周期

- **单次最长运行**:18 小时(`options.keepAliveTimeout` 已拉到上限)。
- **过夜回收**:运行超过 8 小时的工作区在凌晨 04:00–06:00(UTC+8)窗口会被
  平台回收;保活循环负责把它拉回来。
- **实测恢复**:一次真实回收演练中,检出 → 新工作区 → 自注册全程约
  **2 分 13 秒**,固定域名与 API key 全程不变。

## 模型

`/v1/models` 展示的就是 `PROXY_MODELS` 里列的清单。我们账号上,CNB AI 网关
当前暴露三个 id:

| 模型 id | 说明 |
|---|---|
| `deepseek-v4-flash` | 当前实际应答的模型。 |
| `glm-5.3-flash` | 可作为 id 调用,但网关把它路由到 `deepseek-v4-flash`(响应的 `model` 字段返回的是 `deepseek-v4-flash`)。 |
| `kimi-k3` | 同上——当前同样路由到 `deepseek-v4-flash`。 |

也就是说,三个 id 目前都解析到同一个上游模型;额外的名字只是为了让不同
客户端都能对上号。`PROXY_MODELS` 请按你自己账号实际暴露的清单来设。

**已对线上端点实测验证**:SSE 流式与非流式请求、完整 `usage` 聚合(含
`credit` 字段)、原生**函数/工具调用**——带 `tools` 的请求会返回合法的
`tool_calls` 与 `finish_reason: tool_calls`。上下文窗口由上游决定且官方
未文档化,我们不引用无法核实的数字。
